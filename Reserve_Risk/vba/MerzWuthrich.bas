...
'     MW_CumulativeToIncremental - convert a cumulative triangle to incremental
...
'==============================================================================
' Macro: convert a CUMULATIVE triangle to an INCREMENTAL one
'
' Works for any size, including more accident years than development periods
' (more rows than columns). Differences each row over its observed cells; blank
' cells stay blank. Offers to paste the result directly below the cumulative
' block, or at a cell you pick.
'==============================================================================
Public Sub MW_CumulativeToIncremental()
    Dim rng As Range, i As Long, j As Long, nR As Long, nC As Long, raw As Variant
    On Error Resume Next
    ...
    ' Difference each row over its observed cells (blanks left empty).
    Dim out() As Variant: ReDim out(1 To nR, 1 To nC)
    Dim prev As Double, v As Double
    For i = 1 To nR
        prev = 0#
        For j = 1 To nC
