' Attribute VB_Name = "MerzWuthrich"
'==============================================================================
' Merz-Wuthrich (2008) one-year Claims Development Result - ANALYTIC, in VBA
'
' Closed-form one-year reserve risk under the distribution-free chain ladder
' (Mack) model, the companion to Mack (1993) for the ultimate view. No Monte
' Carlo, no matrix algebra - pure arithmetic, so it runs natively in Excel.
'
' Ported from the Python merz_wuthrich.py and validated to reproduce R
' ChainLadder::CDR(MackChainLadder(GenIns)) exactly:
'     one-year SE 1,774,013.8 | ultimate SE 2,441,364.1 | emergence 72.7%
' Run MW_SelfTest to confirm on this machine.
'
' This module uses NO line-continuations, so it is safe to paste as plain text.
'
' Input: a SQUARE range of INCREMENTAL claims (just the value block, no labels),
'        with the unobserved lower-right left blank. Set isCumulative = True if
'        your block holds cumulative figures instead.
'
' WHICH MACRO TO RUN to analyse the triangles in your workbook:
'     MW_RunFromSetup - reads a sheet named "setup" listing each triangle and
'                       runs them all, writing "RiskEmergence_Summary".
'   (For a single triangle, select its value block and run MW_Report.)
'
' Cell functions (UDFs):
'     =MW_EmergenceFactor(range, [sigmaMethod], [isCumulative], [excludeFirstDev])
'     =MW_OneYearSE(range, [sigmaMethod], [isCumulative], [excludeFirstDev])
'     =MW_UltimateSE(range, [sigmaMethod], [isCumulative], [excludeFirstDev])
'     =MW_TotalReserve(range, [isCumulative], [excludeFirstDev])
'   sigmaMethod = "loglinear" (default, matches R ChainLadder) or "mack".
'   excludeFirstDev = TRUE drops the first development column and the immature
'     most-recent accident year (useful for long-tail lines like GL).
'
' Macros:
'     MW_RunFromSetup - batch over the "setup" sheet (main entry point)
'     MW_Portfolio    - aggregate the summary into one portfolio factor (asks rho)
'     MW_Report       - per-AY table + totals + emergence pattern for one triangle
'     MW_Sensitivity  - leave-one-out: which cells/outliers move one-year risk
'     MW_CumulativeToIncremental      - convert one selected cumulative triangle
'     MW_BatchCumulativeToIncremental - convert every cumulative triangle in 'setup'
'     MW_SelfTest     - validates against the GenIns reference numbers
'==============================================================================
Option Explicit
Option Base 1

Public Type MWResult
    n As Long      ' number of development periods (pattern length = n - 1)
    nA As Long     ' number of accident years (rows)
    oneYearSE As Double
    ultimateSE As Double
    totalReserve As Double
    AY_IBNR() As Double
    AY_oneYearSE() As Double
    AY_ultSE() As Double
    patternSE() As Double
    devFactor() As Double   ' chain-ladder factor f_j (j = 1..n-1)
    sigma() As Double       ' Mack's sigma_j  (j = 1..n-1)
    reserveRemaining() As Double ' expected reserve still outstanding at the START of future year s (s = 1..n-1); s = 1 equals the total reserve
    ok As Boolean
    msg As String
End Type

'------------------------------------------------------------------------------
' Core computation. exI/exJ exclude one age-to-age ratio (0 = none); used by the
' sensitivity macro. Returns all headline figures + per-AY and pattern arrays.
'------------------------------------------------------------------------------
Private Function MW_Compute(rng As Range, ByVal sigmaMethod As String, ByVal exI As Long, ByVal exJ As Long, ByVal isCumulative As Boolean, ByVal excludeFirstDev As Boolean) As MWResult
    ' nA = accident years (rows), nD = development periods (columns). Handles
    ' non-square triangles where nA >= nD (e.g. more accident years than dev periods).
    Dim res As MWResult
    Dim nA As Long, nD As Long
    Dim i As Long, j As Long, k As Long, s As Long, c2 As Long, L As Long, cs As Long, firstActive As Long
    Dim raw As Variant

    nA = rng.Rows.Count
    nD = rng.Columns.Count
    If nA < 2 Or nD < 2 Then
        res.ok = False
        res.msg = "Triangle must be at least 2x2."
        MW_Compute = res
        Exit Function
    End If
    If nD > nA Then
        res.ok = False
        res.msg = "Need at least as many accident years (rows) as development periods (columns)."
        MW_Compute = res
        Exit Function
    End If
    raw = rng.Value

    ' --- observed flags and incremental values ---
    Dim Obs() As Boolean: ReDim Obs(1 To nA, 1 To nD)
    Dim Inc() As Double: ReDim Inc(1 To nA, 1 To nD)
    For i = 1 To nA
        For j = 1 To nD
            If Not IsEmpty(raw(i, j)) Then
                If IsNumeric(raw(i, j)) Then
                    Obs(i, j) = True
                    Inc(i, j) = CDbl(raw(i, j))
                End If
            End If
        Next j
    Next i

    ' --- optionally drop the first development column (and the immature most-recent
    '     accident year): merge DP1 dollars into DP2, then shift left ---
    If excludeFirstDev Then
        If nA < 3 Or nD < 3 Then
            res.ok = False
            res.msg = "excludeFirstDev needs at least a 3x3 triangle."
            MW_Compute = res
            Exit Function
        End If
        Dim aRed As Long, dRed As Long, jp As Long
        aRed = nA - 1
        dRed = nD - 1
        Dim rInc() As Double, rObs() As Boolean
        ReDim rInc(1 To aRed, 1 To dRed)
        ReDim rObs(1 To aRed, 1 To dRed)
        For i = 1 To aRed
            For jp = 1 To dRed
                If jp = 1 And Not isCumulative Then
                    If Obs(i, 1) And Obs(i, 2) Then
                        rInc(i, 1) = Inc(i, 1) + Inc(i, 2)
                        rObs(i, 1) = True
                    ElseIf Obs(i, 2) Then
                        rInc(i, 1) = Inc(i, 2)
                        rObs(i, 1) = True
                    End If
                Else
                    If Obs(i, jp + 1) Then
                        rInc(i, jp) = Inc(i, jp + 1)
                        rObs(i, jp) = True
                    End If
                End If
            Next jp
        Next i
        nA = aRed
        nD = dRed
        Inc = rInc
        Obs = rObs
    End If

    ' --- cumulative triangle (over the observed contiguous prefix of each row) ---
    Dim Cum() As Double: ReDim Cum(1 To nA, 1 To nD)
    Dim CObs() As Boolean: ReDim CObs(1 To nA, 1 To nD)
    Dim running As Double
    For i = 1 To nA
        running = 0#
        For j = 1 To nD
            If Not Obs(i, j) Then Exit For
            If isCumulative Then
                Cum(i, j) = Inc(i, j)
            Else
                running = running + Inc(i, j)
                Cum(i, j) = running
            End If
            CObs(i, j) = True
        Next j
    Next i

    ' --- development factors f(j), variance sig2(j), denominator Svol(j)  (j = 1..nD-1) ---
    ' A pair is only usable if Cum(i,j) > 0: Mack's weights are C_ij, so a zero
    ' cell carries zero weight and its age-to-age ratio is undefined (div by 0).
    Dim f() As Double: ReDim f(1 To nD - 1)
    Dim sig2() As Double: ReDim sig2(1 To nD - 1)
    Dim hasSig() As Boolean: ReDim hasSig(1 To nD - 1)
    Dim Svol() As Double: ReDim Svol(1 To nD - 1)
    Dim num As Double, den As Double, cnt As Long, ssq As Double
    For j = 1 To nD - 1
        num = 0#: den = 0#: cnt = 0
        For i = 1 To nA
            If CObs(i, j) And CObs(i, j + 1) And Cum(i, j) > 0 And Not (i = exI And j = exJ) Then
                num = num + Cum(i, j + 1)
                den = den + Cum(i, j)
                cnt = cnt + 1
            End If
        Next i
        If den > 0 Then f(j) = num / den Else f(j) = 1#
        Svol(j) = den
        If cnt >= 2 Then
            ssq = 0#
            For i = 1 To nA
                If CObs(i, j) And CObs(i, j + 1) And Cum(i, j) > 0 And Not (i = exI And j = exJ) Then
                    ssq = ssq + Cum(i, j) * (Cum(i, j + 1) / Cum(i, j) - f(j)) ^ 2
                End If
            Next i
            sig2(j) = ssq / (cnt - 1)
            hasSig(j) = True
        End If
    Next j

    ' --- extrapolate missing sig2. Log-linear fit uses only POSITIVE-variance
    '     columns (flat/zero-variance tail columns keep 0 and would break Log()),
    '     and is overflow-guarded; otherwise Mack's min rule. ---
    Dim posCount As Long: posCount = 0
    For j = 1 To nD - 1
        If hasSig(j) And sig2(j) > 0 Then posCount = posCount + 1
    Next j
    Dim useLoglin As Boolean
    useLoglin = (LCase(sigmaMethod) <> "mack") And (posCount >= 2)
    Dim filledLoglin As Boolean: filledLoglin = False
    If useLoglin Then
        Dim npts As Long, sx As Double, sy As Double, sxx As Double, sxy As Double
        Dim xx As Double, yy As Double, slope As Double, intc As Double, safeExp As Boolean
        npts = 0: sx = 0: sy = 0: sxx = 0: sxy = 0
        For j = 1 To nD - 1
            If hasSig(j) And sig2(j) > 0 Then
                xx = j
                yy = Log(Sqr(sig2(j)))
                npts = npts + 1
                sx = sx + xx
                sy = sy + yy
                sxx = sxx + xx * xx
                sxy = sxy + xx * yy
            End If
        Next j
        slope = (npts * sxy - sx * sy) / (npts * sxx - sx * sx)
        intc = (sy - slope * sx) / npts
        safeExp = True
        For j = 1 To nD - 1
            If Not hasSig(j) Then
                If (intc + slope * j) > 350 Then safeExp = False
            End If
        Next j
        If safeExp Then
            For j = 1 To nD - 1
                If Not hasSig(j) Then
                    sig2(j) = Exp(intc + slope * j) ^ 2
                    hasSig(j) = True
                End If
            Next j
            filledLoglin = True
        End If
    End If
    If Not filledLoglin Then
        For j = 1 To nD - 1
            If Not hasSig(j) Then
                If j >= 3 And sig2(j - 1) > 0 And sig2(j - 2) > 0 Then
                    sig2(j) = WorksheetMin(sig2(j - 1) ^ 2 / sig2(j - 2), sig2(j - 1), sig2(j - 2))
                ElseIf j >= 2 And sig2(j - 1) > 0 Then
                    sig2(j) = sig2(j - 1)
                Else
                    sig2(j) = 0#
                End If
                hasSig(j) = True
            End If
        Next j
    End If

    Dim ratio() As Double: ReDim ratio(1 To nD - 1)
    For j = 1 To nD - 1
        If f(j) <> 0 Then ratio(j) = sig2(j) / f(j) ^ 2 Else ratio(j) = 0#
    Next j

    ' --- full (completed) triangle and ultimates ---
    Dim Full() As Double: ReDim Full(1 To nA, 1 To nD)
    Dim lastObs As Long
    For i = 1 To nA
        lastObs = 0
        For j = 1 To nD
            If CObs(i, j) Then lastObs = j
        Next j
        For j = 1 To nD
            If j <= lastObs Then
                Full(i, j) = Cum(i, j)
            Else
                Full(i, j) = Full(i, j - 1) * f(j - 1)
            End If
        Next j
    Next i
    Dim ult() As Double: ReDim ult(1 To nA)
    For i = 1 To nA
        ult(i) = Full(i, nD)
    Next i

    ' --- newest-diagonal weights alpha(j); the diagonal AY in column j is nA-j+1 ---
    Dim colsum() As Double: ReDim colsum(1 To nD - 1)
    Dim alpha() As Double: ReDim alpha(1 To nD - 1)
    For j = 1 To nD - 1
        den = 0#
        For i = 1 To nA
            If CObs(i, j) Then den = den + Cum(i, j)
        Next i
        colsum(j) = den
        If den > 0 Then alpha(j) = Cum(nA - j + 1, j) / den Else alpha(j) = 0#
    Next j

    ' --- Merz-Wuthrich MSEPs (port of CL_MSEPs). Active accident years are the
    '     last (nD-1) ones; the rest are fully developed (no reserve). ---
    Dim res3() As Double, res5() As Double, res2() As Double
    ReDim res3(1 To nA, 0 To nD - 2)
    ReDim res5(1 To nA, 0 To nD - 2)
    ReDim res2(1 To nA, 0 To nD - 2)
    Dim y As Double, y2 As Double, e As Double
    firstActive = nA - nD + 2
    For i = firstActive To nA
        L = nA - i + 1
        For s = 0 To nD - L - 1
            cs = L + s
            If Full(i, cs) > 0 Then
                res3(i, s) = ult(i) ^ 2 * ratio(cs) / Full(i, cs)
            Else
                res3(i, s) = 0#
            End If
            y = 1#
            For k = L + 1 To L + s
                y = y * (1 - alpha(k))
            Next k
            If Svol(cs) > 0 Then
                e = y * ratio(cs) / Svol(cs)
            Else
                e = 0#
            End If
            For c2 = cs + 1 To nD - 1
                y2 = 1#
                For k = c2 - s + 1 To c2
                    y2 = y2 * (1 - alpha(k))
                Next k
                y2 = y2 * alpha(c2 - s)
                If Svol(c2) > 0 Then e = e + y2 * ratio(c2) / Svol(c2)
            Next c2
            res5(i, s) = e
            res2(i, s) = res3(i, s) + e * ult(i) ^ 2
        Next s
    Next i

    ' --- totals (process + estimation incl. cross-accident-year covariance) ---
    Dim totMSEP() As Double: ReDim totMSEP(0 To nD - 2)
    Dim proc As Double, estt As Double, i1 As Long, mn As Long
    For s = 0 To nD - 2
        proc = 0#
        For i = firstActive To nA
            proc = proc + res3(i, s)
        Next i
        estt = 0#
        For i = firstActive To nA
            For i1 = firstActive To nA
                mn = i
                If i1 < i Then mn = i1
                estt = estt + res5(mn, s) * ult(i) * ult(i1)
            Next i1
        Next i
        totMSEP(s) = proc + estt
    Next s

    ' --- assemble result ---
    ReDim res.AY_IBNR(1 To nA)
    ReDim res.AY_oneYearSE(1 To nA)
    ReDim res.AY_ultSE(1 To nA)
    ReDim res.patternSE(0 To nD - 2)
    Dim msum As Double
    For i = firstActive To nA
        L = nA - i + 1
        res.AY_IBNR(i) = ult(i) - Full(i, L)
        res.AY_oneYearSE(i) = Sqr(res2(i, 0))
        msum = 0#
        For s = 0 To nD - L - 1
            msum = msum + res2(i, s)
        Next s
        res.AY_ultSE(i) = Sqr(msum)
    Next i
    Dim ultMSEP As Double: ultMSEP = 0#
    For s = 0 To nD - 2
        res.patternSE(s) = Sqr(totMSEP(s))
        ultMSEP = ultMSEP + totMSEP(s)
    Next s
    ReDim res.devFactor(1 To nD - 1)
    ReDim res.sigma(1 To nD - 1)
    For j = 1 To nD - 1
        res.devFactor(j) = f(j)
        res.sigma(j) = Sqr(sig2(j))
    Next j
    ' Remaining reserve at the START of future year s: for each active accident
    ' year the expected cumulative position after s-1 more years is
    ' Full(i, L+s-1), so the outstanding piece is ult - Full(i, L+s-1).
    ReDim res.reserveRemaining(1 To nD - 1)
    Dim remRes As Double, colAt As Long
    For s = 1 To nD - 1
        remRes = 0#
        For i = firstActive To nA
            L = nA - i + 1
            colAt = L + s - 1
            If colAt > nD Then colAt = nD
            remRes = remRes + (ult(i) - Full(i, colAt))
        Next i
        res.reserveRemaining(s) = remRes
    Next s
    res.n = nD
    res.nA = nA
    res.oneYearSE = Sqr(totMSEP(0))
    res.ultimateSE = Sqr(ultMSEP)
    res.totalReserve = 0#
    For i = 1 To nA
        res.totalReserve = res.totalReserve + res.AY_IBNR(i)
    Next i
    res.ok = True
    MW_Compute = res
End Function

Private Function WorksheetMin(a As Double, b As Double, c As Double) As Double
    Dim m As Double: m = a
    If b < m Then m = b
    If c < m Then m = c
    WorksheetMin = m
End Function

'==============================================================================
' Cell UDFs
'==============================================================================
Public Function MW_EmergenceFactor(rng As Range, Optional sigmaMethod As String = "loglinear", Optional isCumulative As Boolean = False, Optional excludeFirstDev As Boolean = False) As Variant
    Dim r As MWResult
    r = MW_Compute(rng, sigmaMethod, 0, 0, isCumulative, excludeFirstDev)
    If Not r.ok Then MW_EmergenceFactor = r.msg: Exit Function
    If r.ultimateSE = 0 Then MW_EmergenceFactor = "ultimate SE is zero": Exit Function
    MW_EmergenceFactor = r.oneYearSE / r.ultimateSE
End Function

Public Function MW_OneYearSE(rng As Range, Optional sigmaMethod As String = "loglinear", Optional isCumulative As Boolean = False, Optional excludeFirstDev As Boolean = False) As Variant
    Dim r As MWResult
    r = MW_Compute(rng, sigmaMethod, 0, 0, isCumulative, excludeFirstDev)
    If Not r.ok Then MW_OneYearSE = r.msg: Exit Function
    MW_OneYearSE = r.oneYearSE
End Function

Public Function MW_UltimateSE(rng As Range, Optional sigmaMethod As String = "loglinear", Optional isCumulative As Boolean = False, Optional excludeFirstDev As Boolean = False) As Variant
    Dim r As MWResult
    r = MW_Compute(rng, sigmaMethod, 0, 0, isCumulative, excludeFirstDev)
    If Not r.ok Then MW_UltimateSE = r.msg: Exit Function
    MW_UltimateSE = r.ultimateSE
End Function

Public Function MW_TotalReserve(rng As Range, Optional isCumulative As Boolean = False, Optional excludeFirstDev As Boolean = False) As Variant
    Dim r As MWResult
    r = MW_Compute(rng, "loglinear", 0, 0, isCumulative, excludeFirstDev)
    If Not r.ok Then MW_TotalReserve = r.msg: Exit Function
    MW_TotalReserve = r.totalReserve
End Function

'==============================================================================
' Macro: batch run from a "setup" sheet  (MAIN ENTRY POINT)
'
' The "setup" sheet lists triangles to analyse (headers in row 1, data from
' row 2):
'     A: Worksheet       - name of the sheet holding the triangle  (required)
'     B: Range           - value block, e.g. "C3:L12" (blank = sheet's UsedRange)
'     C: Cumulative      - "Y"/"N" (default N = incremental)
'     D: SigmaMethod     - "loglinear" (default) or "mack"
'     E: ExcludeFirstDev - "Y"/"N" (default N)
' Writes one row per triangle to "RiskEmergence_Summary": headline figures plus
' the emergence-by-year pattern. Bad rows are flagged, not fatal.
'
' It ALSO writes, on each triangle worksheet itself:
'     row 2: risk emergence per future year (that year's CDR SD / ultimate SD)
'     row 3: remaining reserve at the START of that future year (volume weights)
' aligned with the triangle's columns (labels in column A). If the triangle sits
' too high for that, rows are inserted above it first (the row directly above the
' block is kept free for any header labels), and the shifted triangle address is
' written back to the setup sheet. Rows 2-3 are overwritten on every re-run.
'==============================================================================
Public Sub MW_RunFromSetup()
    Dim setupWs As Worksheet, tgt As Worksheet, ws As Worksheet
    Dim lastRow As Long, r As Long, k As Long, total As Long, maxSteps As Long
    Dim wsName As String, rngStr As String, cumStr As String, sigM As String, exclStr As String
    Dim blk As Range, res As MWResult

    On Error Resume Next
    Set setupWs = ThisWorkbook.Worksheets("setup")
    On Error GoTo 0
    If setupWs Is Nothing Then
        MsgBox "No worksheet named 'setup' was found.", vbExclamation, "MW_RunFromSetup"
        Exit Sub
    End If

    lastRow = setupWs.Cells(setupWs.Rows.Count, 1).End(xlUp).Row
    If lastRow < 2 Then
        MsgBox "List the worksheet names in column A of 'setup' (from row 2 down).", vbExclamation, "MW_RunFromSetup"
        Exit Sub
    End If
    total = lastRow - 1

    Dim names() As String, ns() As Long, nAs() As Long, reserves() As Double
    Dim ultSE() As Double, oySE() As Double, ef() As Double, errs() As String, patterns() As Variant
    Dim blks() As Range, tgts() As Worksheet, setupRows() As Long
    Dim isCums() As Boolean, sigMs() As String, excls() As Boolean
    ReDim names(1 To total): ReDim ns(1 To total): ReDim nAs(1 To total): ReDim reserves(1 To total)
    ReDim ultSE(1 To total): ReDim oySE(1 To total): ReDim ef(1 To total)
    ReDim errs(1 To total): ReDim patterns(1 To total)
    ReDim blks(1 To total): ReDim tgts(1 To total): ReDim setupRows(1 To total)
    ReDim isCums(1 To total): ReDim sigMs(1 To total): ReDim excls(1 To total)

    ' --- pass 1: resolve every worksheet and range up front. Range objects track
    '     later row insertions, so triangles on a shared sheet stay in sync. ---
    k = 0
    For r = 2 To lastRow
        wsName = Trim(CStr(setupWs.Cells(r, 1).Value))
        If wsName <> "" Then
            k = k + 1
            names(k) = wsName
            setupRows(k) = r
            rngStr = Trim(CStr(setupWs.Cells(r, 2).Value))
            cumStr = UCase(Trim(CStr(setupWs.Cells(r, 3).Value)))
            sigM = LCase(Trim(CStr(setupWs.Cells(r, 4).Value)))
            If sigM = "" Then sigM = "loglinear"
            exclStr = UCase(Trim(CStr(setupWs.Cells(r, 5).Value)))
            isCums(k) = (cumStr = "Y" Or cumStr = "YES" Or cumStr = "TRUE" Or cumStr = "1")
            excls(k) = (exclStr = "Y" Or exclStr = "YES" Or exclStr = "TRUE" Or exclStr = "1")
            sigMs(k) = sigM

            Set tgt = Nothing
            On Error Resume Next
            Set tgt = ThisWorkbook.Worksheets(wsName)
            On Error GoTo 0
            If tgt Is Nothing Then
                errs(k) = "worksheet not found"
            Else
                Set blk = Nothing
                On Error Resume Next
                If rngStr = "" Then Set blk = tgt.UsedRange Else Set blk = tgt.Range(rngStr)
                On Error GoTo 0
                If blk Is Nothing Then
                    errs(k) = "invalid range '" & rngStr & "'"
                Else
                    Set tgts(k) = tgt
                    Set blks(k) = blk
                End If
            End If
        End If
    Next r

    If k = 0 Then
        MsgBox "No worksheet names found in 'setup'.", vbExclamation
        Exit Sub
    End If

    ' --- pass 2: make room for the row-2/3 strips. The triangle must start at
    '     row 5 or below (rows 2-3 hold the strips; the row directly above the
    '     block stays free for header labels). Insert rows above if needed, then
    '     pin the (possibly shifted) address back onto the setup sheet so re-runs
    '     never pick the strips up as part of the triangle. ---
    Dim i As Long, needRows As Long
    For i = 1 To k
        If errs(i) = "" Then
            If blks(i).Row < 5 Then
                needRows = 5 - blks(i).Row
                tgts(i).Rows("1:" & needRows).Insert
            End If
            setupWs.Cells(setupRows(i), 2).Value = blks(i).Address(False, False)
        End If
    Next i

    ' --- pass 3: compute each triangle, write its row-2/3 strips, collect summary ---
    maxSteps = 0
    For i = 1 To k
        If errs(i) = "" Then
            res = MW_Compute(blks(i), sigMs(i), 0, 0, isCums(i), excls(i))
            If Not res.ok Then
                errs(i) = res.msg
            ElseIf res.ultimateSE = 0 Then
                errs(i) = "ultimate SE is zero (degenerate triangle)"
            Else
                ns(i) = res.n
                nAs(i) = res.nA
                reserves(i) = res.totalReserve
                ultSE(i) = res.ultimateSE
                oySE(i) = res.oneYearSE
                ef(i) = res.oneYearSE / res.ultimateSE
                patterns(i) = res.patternSE
                If res.n - 1 > maxSteps Then maxSteps = res.n - 1
                MW_WriteEmergenceRows tgts(i), blks(i), res
            End If
        End If
    Next i

    Set ws = MW_FreshSheet("RiskEmergence_Summary")
    ws.Range("A1").Value = "Risk emergence (analytic Merz-Wuthrich) - batch from 'setup'"
    ws.Range("A1").Font.Bold = True
    ws.Range("A2").Value = "Aggregate emergence factor (col F) = One-year SE / Ultimate SE = EF Yr1 by definition. EF Yr columns = per-development-period emergence (each = that future period's CDR SD / Ultimate SD; squares sum to 100%)."
    ws.Cells(3, 1).Value = "Worksheet": ws.Cells(3, 2).Value = "AY x DP"
    ws.Cells(3, 3).Value = "Total reserve": ws.Cells(3, 4).Value = "Ultimate SE"
    ws.Cells(3, 5).Value = "One-year SE": ws.Cells(3, 6).Value = "Emergence factor"
    ws.Cells(3, 7).Value = "Status"
    Dim baseCol As Long: baseCol = 8
    Dim sCol As Long
    For sCol = 1 To maxSteps
        ws.Cells(3, baseCol + sCol - 1).Value = "EF Yr" & sCol
    Next sCol
    ws.Range(ws.Cells(3, 1), ws.Cells(3, baseCol + maxSteps - 1)).Font.Bold = True

    Dim outRow As Long, p As Variant, sIdx As Long
    outRow = 4
    For i = 1 To k
        ws.Cells(outRow, 1).Value = names(i)
        If errs(i) <> "" Then
            ws.Cells(outRow, 7).Value = "ERROR: " & errs(i)
        Else
            ws.Cells(outRow, 2).Value = nAs(i) & " x " & ns(i)
            ws.Cells(outRow, 3).Value = reserves(i)
            ws.Cells(outRow, 4).Value = ultSE(i)
            ws.Cells(outRow, 5).Value = oySE(i)
            ws.Cells(outRow, 6).Value = ef(i): ws.Cells(outRow, 6).NumberFormat = "0.0%"
            ws.Cells(outRow, 7).Value = "OK"
            p = patterns(i)
            For sIdx = 0 To ns(i) - 2
                ws.Cells(outRow, baseCol + sIdx).Value = p(sIdx) / ultSE(i)
                ws.Cells(outRow, baseCol + sIdx).NumberFormat = "0.0%"
            Next sIdx
        End If
        outRow = outRow + 1
    Next i
    ws.Columns.AutoFit
    ws.Activate
    MsgBox "Processed " & k & " worksheet(s) listed in 'setup'." & vbCrLf & "Each triangle sheet now holds the per-year emergence (row 2) and the remaining reserve at the start of each year (row 3).", vbInformation, "MW_RunFromSetup"
End Sub

'------------------------------------------------------------------------------
' Write the per-year strips onto a triangle worksheet (called by MW_RunFromSetup;
' the caller guarantees the triangle block starts at row 5 or below):
'     row 2: risk emergence per future year  (year s CDR SD / ultimate SD)
'     row 3: remaining reserve at the START of that year (for volume weighting;
'            year 1 = the total reserve)
' Values sit in the triangle's own columns (year 1 under the block's first
' column), labels in column A. Rows 2-3 are cleared first, so re-running simply
' overwrites the previous strips.
'------------------------------------------------------------------------------
Private Sub MW_WriteEmergenceRows(tgt As Worksheet, blk As Range, res As MWResult)
    Dim s As Long, c0 As Long
    tgt.Rows("2:3").ClearContents
    c0 = blk.Column
    If c0 < 2 Then c0 = 2
    tgt.Cells(2, 1).Value = "Risk emergence by future year (Yr1, Yr2, ...):"
    tgt.Cells(2, 1).Font.Bold = True
    tgt.Cells(3, 1).Value = "Remaining reserve at start of year:"
    tgt.Cells(3, 1).Font.Bold = True
    For s = 1 To res.n - 1
        tgt.Cells(2, c0 + s - 1).Value = res.patternSE(s - 1) / res.ultimateSE
        tgt.Cells(2, c0 + s - 1).NumberFormat = "0.0%"
        tgt.Cells(3, c0 + s - 1).Value = res.reserveRemaining(s)
        tgt.Cells(3, c0 + s - 1).NumberFormat = "#,##0"
    Next s
End Sub

'==============================================================================
' Macro: portfolio aggregation across LoBs (single correlation rho)
'
' Reads per-LoB one-year SE and ultimate SE from "RiskEmergence_Summary" and
' aggregates each with a single pairwise rho, then divides. Emergence factors are
' ratios of SDs, so the dollar SEs are aggregated WITH correlation - you cannot
' average the factors directly. rho largely cancels in the ratio, so the factor
' is robust; the independence / full-correlation bookends are shown too.
'==============================================================================
Public Sub MW_Portfolio()
    Dim ws As Worksheet, hdr As Long, c As Long
    Dim colWs As Long, colOY As Long, colUL As Long, colStat As Long

    On Error Resume Next
    Set ws = ThisWorkbook.Worksheets("RiskEmergence_Summary")
    On Error GoTo 0
    If ws Is Nothing Then
        MsgBox "Run MW_RunFromSetup first (no 'RiskEmergence_Summary' sheet).", vbExclamation
        Exit Sub
    End If

    hdr = 3
    For c = 1 To 30
        Select Case CStr(ws.Cells(hdr, c).Value)
            Case "Worksheet": colWs = c
            Case "One-year SE": colOY = c
            Case "Ultimate SE": colUL = c
            Case "Status": colStat = c
        End Select
    Next c
    If colOY = 0 Or colUL = 0 Then
        MsgBox "Could not find the 'One-year SE' / 'Ultimate SE' columns.", vbExclamation
        Exit Sub
    End If

    Dim rhoStr As String, rho As Double
    rhoStr = InputBox("Correlation rho between LoBs (0 = independent, 1 = fully correlated):", "Portfolio aggregation", "0.25")
    If rhoStr = "" Then Exit Sub
    If Not IsNumeric(rhoStr) Then MsgBox "rho must be numeric.", vbExclamation: Exit Sub
    rho = CDbl(rhoStr)
    If rho < 0 Or rho > 1 Then MsgBox "rho must be between 0 and 1.", vbExclamation: Exit Sub

    Dim r As Long, lastRow As Long, cnt As Long
    Dim sumO As Double, sumU As Double, sumOO As Double, sumUU As Double, oVal As Double, uVal As Double
    r = hdr + 1
    Do While Trim(CStr(ws.Cells(r, colWs).Value)) <> ""
        If colStat = 0 Or UCase(Trim(CStr(ws.Cells(r, colStat).Value))) = "OK" Then
            If IsNumeric(ws.Cells(r, colOY).Value) And IsNumeric(ws.Cells(r, colUL).Value) Then
                oVal = CDbl(ws.Cells(r, colOY).Value)
                uVal = CDbl(ws.Cells(r, colUL).Value)
                sumO = sumO + oVal
                sumU = sumU + uVal
                sumOO = sumOO + oVal * oVal
                sumUU = sumUU + uVal * uVal
                cnt = cnt + 1
            End If
        End If
        lastRow = r
        r = r + 1
    Loop
    If cnt = 0 Then MsgBox "No OK rows with numeric SEs found.", vbExclamation: Exit Sub

    Dim portO As Double, portU As Double
    portO = Sqr(rho * sumO * sumO + (1 - rho) * sumOO)
    portU = Sqr(rho * sumU * sumU + (1 - rho) * sumUU)

    Dim w As Long: w = lastRow + 2
    ws.Cells(w, 1).Value = "Portfolio (rho = " & Format(rho, "0.00") & ", " & cnt & " LoBs)"
    ws.Cells(w, 1).Font.Bold = True
    ws.Cells(w + 1, 1).Value = "Portfolio one-year SE": ws.Cells(w + 1, 2).Value = portO
    ws.Cells(w + 2, 1).Value = "Portfolio ultimate SE": ws.Cells(w + 2, 2).Value = portU
    ws.Cells(w + 3, 1).Value = "Portfolio emergence factor": ws.Cells(w + 3, 2).Value = portO / portU
    ws.Cells(w + 3, 2).NumberFormat = "0.0%"
    ws.Cells(w + 4, 1).Value = "  bookend: independent (rho=0)"
    ws.Cells(w + 4, 2).Value = Sqr(sumOO) / Sqr(sumUU): ws.Cells(w + 4, 2).NumberFormat = "0.0%"
    ws.Cells(w + 5, 1).Value = "  bookend: fully correlated (rho=1)"
    ws.Cells(w + 5, 2).Value = sumO / sumU: ws.Cells(w + 5, 2).NumberFormat = "0.0%"
    ws.Cells(w + 6, 1).Value = "Diversification benefit (one-year)"
    ws.Cells(w + 6, 2).Value = 1 - portO / sumO: ws.Cells(w + 6, 2).NumberFormat = "0.0%"
    ws.Columns("A:B").AutoFit
    ws.Activate
    MsgBox "Portfolio emergence factor = " & Format(portO / portU, "0.0%") & "  (rho " & Format(rho, "0.00") & ", " & cnt & " LoBs)", vbInformation, "MW_Portfolio"
End Sub

'==============================================================================
' Macro: full report (per-AY table + totals + emergence pattern) for one triangle
'==============================================================================
Public Sub MW_Report()
    Dim rng As Range, isCum As Boolean, exclFirst As Boolean, r As MWResult
    Dim ws As Worksheet, i As Long, j As Long, s As Long, rowOut As Long
    On Error Resume Next
    Set rng = Application.InputBox("Select the incremental triangle value block (no labels):", "MW Report", Type:=8)
    On Error GoTo 0
    If rng Is Nothing Then Exit Sub
    isCum = (MsgBox("Is the selected block CUMULATIVE? (No = incremental)", vbYesNo + vbQuestion, "Orientation") = vbYes)
    exclFirst = (MsgBox("Exclude the first development column (and the immature most-recent accident year)?", vbYesNo + vbQuestion, "Exclude first dev") = vbYes)

    r = MW_Compute(rng, "loglinear", 0, 0, isCum, exclFirst)
    If Not r.ok Then MsgBox r.msg, vbExclamation: Exit Sub
    If r.ultimateSE = 0 Then MsgBox "Ultimate SE is zero (degenerate triangle) - nothing to report.", vbExclamation: Exit Sub

    Set ws = MW_FreshSheet("MW_Results")
    ws.Range("A1").Value = "Merz-Wuthrich one-year reserve risk"
    ws.Range("A1").Font.Bold = True
    ws.Range("A3").Value = "Emergence factor (one-year / ultimate)"
    ws.Range("B3").Value = r.oneYearSE / r.ultimateSE: ws.Range("B3").NumberFormat = "0.0%"
    ws.Range("A4").Value = "One-year SE": ws.Range("B4").Value = r.oneYearSE
    ws.Range("A5").Value = "Ultimate SE": ws.Range("B5").Value = r.ultimateSE
    ws.Range("A6").Value = "Total reserve": ws.Range("B6").Value = r.totalReserve

    ws.Range("A8").Value = "AY": ws.Range("B8").Value = "IBNR"
    ws.Range("C8").Value = "One-year CDR SE": ws.Range("D8").Value = "Ultimate Mack SE"
    ws.Range("A8:D8").Font.Bold = True
    rowOut = 9
    For i = 1 To r.nA
        ws.Cells(rowOut, 1).Value = i
        ws.Cells(rowOut, 2).Value = r.AY_IBNR(i)
        ws.Cells(rowOut, 3).Value = r.AY_oneYearSE(i)
        ws.Cells(rowOut, 4).Value = r.AY_ultSE(i)
        rowOut = rowOut + 1
    Next i
    ws.Cells(rowOut, 1).Value = "Total"
    ws.Cells(rowOut, 2).Value = r.totalReserve
    ws.Cells(rowOut, 3).Value = r.oneYearSE
    ws.Cells(rowOut, 4).Value = r.ultimateSE
    ws.Cells(rowOut, 1).Resize(1, 4).Font.Bold = True

    rowOut = rowOut + 2
    ws.Cells(rowOut, 1).Value = "Risk emergence by future development period": ws.Cells(rowOut, 1).Font.Bold = True
    rowOut = rowOut + 1
    ws.Cells(rowOut, 1).Value = "Future period": ws.Cells(rowOut, 2).Value = "Total CDR SE"
    ws.Cells(rowOut, 3).Value = "Emergence (CDR SD / ult SD)": ws.Cells(rowOut, 1).Resize(1, 3).Font.Bold = True
    For s = 0 To r.n - 2
        rowOut = rowOut + 1
        ws.Cells(rowOut, 1).Value = s + 1
        ws.Cells(rowOut, 2).Value = r.patternSE(s)
        ws.Cells(rowOut, 3).Value = r.patternSE(s) / r.ultimateSE
        ws.Cells(rowOut, 3).NumberFormat = "0.0%"
    Next s

    rowOut = rowOut + 2
    ws.Cells(rowOut, 1).Value = "Development parameters": ws.Cells(rowOut, 1).Font.Bold = True
    rowOut = rowOut + 1
    ws.Cells(rowOut, 1).Value = "Dev period": ws.Cells(rowOut, 2).Value = "Factor f": ws.Cells(rowOut, 3).Value = "Sigma"
    ws.Cells(rowOut, 1).Resize(1, 3).Font.Bold = True
    For j = 1 To r.n - 1
        rowOut = rowOut + 1
        ws.Cells(rowOut, 1).Value = j
        ws.Cells(rowOut, 2).Value = r.devFactor(j)
        ws.Cells(rowOut, 3).Value = r.sigma(j)
    Next j

    ws.Columns("A:D").AutoFit
    ws.Activate
End Sub

'==============================================================================
' Macro: leave-one-out outlier sensitivity (impact on the one-year SE)
'==============================================================================
Public Sub MW_Sensitivity()
    Dim rng As Range, isCum As Boolean, exclFirst As Boolean, base As MWResult, alt As MWResult
    Dim nA As Long, nD As Long, i As Long, j As Long, ws As Worksheet, rowOut As Long
    On Error Resume Next
    Set rng = Application.InputBox("Select the incremental triangle value block (no labels):", "MW Sensitivity", Type:=8)
    On Error GoTo 0
    If rng Is Nothing Then Exit Sub
    isCum = (MsgBox("Is the selected block CUMULATIVE? (No = incremental)", vbYesNo + vbQuestion, "Orientation") = vbYes)
    exclFirst = (MsgBox("Exclude the first development column (and the immature most-recent accident year)?", vbYesNo + vbQuestion, "Exclude first dev") = vbYes)

    base = MW_Compute(rng, "loglinear", 0, 0, isCum, exclFirst)
    If Not base.ok Then MsgBox base.msg, vbExclamation: Exit Sub
    If base.ultimateSE = 0 Then MsgBox "Ultimate SE is zero (degenerate triangle) - nothing to rank.", vbExclamation: Exit Sub
    nA = base.nA
    nD = base.n

    Dim colCnt() As Long: ReDim colCnt(1 To nD - 1)
    For j = 1 To nD - 1
        For i = 1 To nA - j
            colCnt(j) = colCnt(j) + 1
        Next i
    Next j

    Set ws = MW_FreshSheet("MW_Sensitivity")
    ws.Range("A1").Value = "Leave-one-out sensitivity (impact of excluding each age-to-age ratio)"
    ws.Range("A1").Font.Bold = True
    ws.Range("A3:F3").Value = Array("AY", "Dev period", "d_oneYear_SE", "d_ultimate_SE", "d_reserve", "d_emergence")
    ws.Range("A3:F3").Font.Bold = True
    rowOut = 4
    Dim baseEF As Double: baseEF = base.oneYearSE / base.ultimateSE
    For i = 1 To nA
        For j = 1 To nD - 1
            If i <= nA - j And colCnt(j) > 1 Then
                alt = MW_Compute(rng, "loglinear", i, j, isCum, exclFirst)
                If alt.ok Then
                    ws.Cells(rowOut, 1).Value = i
                    ws.Cells(rowOut, 2).Value = j
                    ws.Cells(rowOut, 3).Value = alt.oneYearSE - base.oneYearSE
                    ws.Cells(rowOut, 4).Value = alt.ultimateSE - base.ultimateSE
                    ws.Cells(rowOut, 5).Value = alt.totalReserve - base.totalReserve
                    If alt.ultimateSE > 0 Then ws.Cells(rowOut, 6).Value = (alt.oneYearSE / alt.ultimateSE) - baseEF
                    rowOut = rowOut + 1
                End If
            End If
        Next j
    Next i
    If rowOut > 4 Then
        ws.Range("H4:H" & rowOut - 1).Formula = "=ABS(C4)"
        ws.Range("A3:H" & rowOut - 1).Sort Key1:=ws.Range("H3"), Order1:=xlDescending, Header:=xlYes
        ws.Columns("H").Delete
    End If
    ws.Columns("A:F").AutoFit
    ws.Activate
End Sub

'==============================================================================
' Cumulative -> incremental conversion
'
' IncrementalOf: shared core. Differences each row over its observed cells
'   (blanks left empty). Works for any size, incl. more accident years than
'   development periods (rows > columns).
' MW_CumulativeToIncremental: convert ONE triangle you select (prompts).
' MW_BatchCumulativeToIncremental: convert EVERY cumulative triangle listed on
'   the "setup" sheet at once, writing each incremental copy below its block.
'==============================================================================
Private Function IncrementalOf(rng As Range) As Variant
    Dim nR As Long, nC As Long, i As Long, j As Long, raw As Variant
    Dim out() As Variant, prev As Double, v As Double
    nR = rng.Rows.Count
    nC = rng.Columns.Count
    ReDim out(1 To nR, 1 To nC)
    If nR = 1 And nC = 1 Then
        If IsNumeric(rng.Value) Then out(1, 1) = CDbl(rng.Value)
        IncrementalOf = out
        Exit Function
    End If
    raw = rng.Value
    For i = 1 To nR
        prev = 0#
        For j = 1 To nC
            If Not IsEmpty(raw(i, j)) And IsNumeric(raw(i, j)) Then
                v = CDbl(raw(i, j))
                out(i, j) = v - prev
                prev = v
            End If
        Next j
    Next i
    IncrementalOf = out
End Function

' Write the incremental form of srcBlk: a bold "Incremental triangle" label at
' labelCell and the values at dataTL.
Private Sub WriteIncremental(srcBlk As Range, labelCell As Range, dataTL As Range)
    labelCell.Value = "Incremental triangle"
    labelCell.Font.Bold = True
    dataTL.Resize(srcBlk.Rows.Count, srcBlk.Columns.Count).Value = IncrementalOf(srcBlk)
End Sub

Public Sub MW_CumulativeToIncremental()
    Dim rng As Range, nR As Long, nC As Long
    On Error Resume Next
    Set rng = Application.InputBox("Select the CUMULATIVE triangle value block (numbers only, no labels):", "Cumulative to incremental", Type:=8)
    On Error GoTo 0
    If rng Is Nothing Then Exit Sub
    If rng.Areas.Count > 1 Then MsgBox "Please select a single contiguous block.", vbExclamation: Exit Sub
    nR = rng.Rows.Count
    nC = rng.Columns.Count
    If nR * nC < 2 Then MsgBox "Select a triangle (more than one cell).", vbExclamation: Exit Sub

    Dim below As Boolean
    below = (MsgBox("Paste the incremental triangle directly below the cumulative one?" & vbCrLf & "(No = pick a target cell)", vbYesNo + vbQuestion, "Output location") = vbYes)

    Dim topLeft As Range, labelCell As Range, dataTL As Range, pick As Range
    Set topLeft = rng.Cells(1, 1)
    If below Then
        Set labelCell = topLeft.Offset(nR + 1, 0)
        Set dataTL = topLeft.Offset(nR + 2, 0)
    Else
        On Error Resume Next
        Set pick = Application.InputBox("Select the TOP-LEFT cell for the incremental triangle:", "Output location", Type:=8)
        On Error GoTo 0
        If pick Is Nothing Then Exit Sub
        Set labelCell = pick.Cells(1, 1)
        Set dataTL = labelCell.Offset(1, 0)
    End If

    WriteIncremental rng, labelCell, dataTL
    MsgBox "Created an incremental " & nR & " x " & nC & " triangle.", vbInformation, "Cumulative to incremental"
End Sub

'==============================================================================
' Macro: batch convert every CUMULATIVE triangle on the "setup" sheet
'
' For each setup row whose Cumulative column is Y, writes an incremental copy
' directly below the cumulative block in that worksheet. Rows that are already
' incremental are skipped. The 'setup' sheet itself is not modified.
'==============================================================================
Public Sub MW_BatchCumulativeToIncremental()
    Dim setupWs As Worksheet, tgt As Worksheet
    Dim lastRow As Long, r As Long, done As Long, skipped As Long, failed As Long
    Dim wsName As String, rngStr As String, cumStr As String
    Dim isCum As Boolean, blk As Range, topLeft As Range, nR As Long

    On Error Resume Next
    Set setupWs = ThisWorkbook.Worksheets("setup")
    On Error GoTo 0
    If setupWs Is Nothing Then
        MsgBox "No worksheet named 'setup' was found.", vbExclamation, "MW_BatchCumulativeToIncremental"
        Exit Sub
    End If

    lastRow = setupWs.Cells(setupWs.Rows.Count, 1).End(xlUp).Row
    For r = 2 To lastRow
        wsName = Trim(CStr(setupWs.Cells(r, 1).Value))
        If wsName <> "" Then
            cumStr = UCase(Trim(CStr(setupWs.Cells(r, 3).Value)))
            isCum = (cumStr = "Y" Or cumStr = "YES" Or cumStr = "TRUE" Or cumStr = "1")
            If Not isCum Then
                skipped = skipped + 1
            Else
                rngStr = Trim(CStr(setupWs.Cells(r, 2).Value))
                Set tgt = Nothing
                On Error Resume Next
                Set tgt = ThisWorkbook.Worksheets(wsName)
                On Error GoTo 0
                If tgt Is Nothing Then
                    failed = failed + 1
                Else
                    Set blk = Nothing
                    On Error Resume Next
                    If rngStr = "" Then Set blk = tgt.UsedRange Else Set blk = tgt.Range(rngStr)
                    On Error GoTo 0
                    If blk Is Nothing Then
                        failed = failed + 1
                    Else
                        Set topLeft = blk.Cells(1, 1)
                        nR = blk.Rows.Count
                        WriteIncremental blk, topLeft.Offset(nR + 1, 0), topLeft.Offset(nR + 2, 0)
                        done = done + 1
                    End If
                End If
            End If
        End If
    Next r

    MsgBox "Converted " & done & " cumulative triangle(s) to incremental (written below each block)." & vbCrLf & "Skipped " & skipped & " already-incremental row(s); " & failed & " error(s).", vbInformation, "MW_BatchCumulativeToIncremental"
End Sub

'==============================================================================
' Self-test: build the GenIns triangle in code and check the answer
'==============================================================================
Public Sub MW_SelfTest()
    Dim g(1 To 10) As Variant, ws As Worksheet, i As Long, j As Long, r As MWResult, verdict As String
    g(1) = Array(357848, 766940, 610542, 482940, 527326, 574398, 146342, 139950, 227229, 67948)
    g(2) = Array(352118, 884021, 933894, 1183289, 445745, 320996, 527804, 266172, 425046)
    g(3) = Array(290507, 1001799, 926219, 1016654, 750816, 146923, 495992, 280405)
    g(4) = Array(310608, 1108250, 776189, 1562400, 272482, 352053, 206286)
    g(5) = Array(443160, 693190, 991983, 769488, 504851, 470639)
    g(6) = Array(396132, 937085, 847498, 805037, 705960)
    g(7) = Array(440832, 847631, 1131398, 1063269)
    g(8) = Array(359480, 1061648, 1443370)
    g(9) = Array(376686, 986608)
    g(10) = Array(344014)
    Set ws = MW_FreshSheet("MW_SelfTest")
    For i = 1 To 10
        For j = 1 To 11 - i
            ws.Cells(i, j).Value = g(i)(j - 1)
        Next j
    Next i
    r = MW_Compute(ws.Range(ws.Cells(1, 1), ws.Cells(10, 10)), "loglinear", 0, 0, False, False)
    Dim okOY As Boolean, okUL As Boolean
    okOY = Abs(r.oneYearSE - 1774013.8) < 1#
    okUL = Abs(r.ultimateSE - 2441364.1) < 1#
    If okOY And okUL Then verdict = "PASS - matches reference." Else verdict = "FAIL - check the port."
    MsgBox "GenIns self-test" & vbCrLf & "One-year SE : " & Format(r.oneYearSE, "#,##0.0") & "  (ref 1,774,013.8)" & vbCrLf & "Ultimate SE : " & Format(r.ultimateSE, "#,##0.0") & "  (ref 2,441,364.1)" & vbCrLf & "Emergence   : " & Format(r.oneYearSE / r.ultimateSE, "0.0%") & vbCrLf & vbCrLf & verdict, IIf(okOY And okUL, vbInformation, vbExclamation), "MW_SelfTest"
End Sub

Private Function MW_FreshSheet(nm As String) As Worksheet
    ' Reuse the sheet if it already exists (clearing old contents), else create it.
    ' Reusing preserves a pre-built / pre-positioned sheet rather than deleting it.
    Dim ws As Worksheet
    On Error Resume Next
    Set ws = ThisWorkbook.Worksheets(nm)
    On Error GoTo 0
    If ws Is Nothing Then
        Set ws = ThisWorkbook.Worksheets.Add
        ws.Name = nm
    Else
        ws.Cells.Clear
    End If
    Set MW_FreshSheet = ws
End Function
